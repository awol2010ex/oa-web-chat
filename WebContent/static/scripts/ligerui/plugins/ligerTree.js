    ...
    $.ligerui.controls.Tree.ligerExtend($.ligerui.core.UIComponent, {
        ...
        update: function (domnode, newnodedata)
        {
            ...
            for (var attr in newnodedata)
            {
                ...
                if (attr == p.tipFieldName)
                {
                    $("> .l-body > span", domnode).attr("title",newnodedata[attr]);
                }
            }
        },
        //增加节点集合
        ...
        _getTreeHTMLByData: function (data, outlineLevel, isLast, isExpand)
        {
            ...
            for (var i = 0; i < data.length; i++)
            {
                ...
                treehtmlarr.push("<span title='"+(o[p.tipFieldName]?o[p.tipFieldName]:o[p.textFieldName]) +"' >" + o[p.textFieldName] + '</span></div>');
                if (g.hasChildren(o))
                {
                    var isLastNew = [];
